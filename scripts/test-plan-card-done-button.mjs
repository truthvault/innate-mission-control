...
const clientMustHave = [
  ...
  ['card Done action is explicitly checkmark-first for workshop use', 'task.done ? "↩ Undo" : "✓ Done"'],
  ...
  ['done card is visually struck through', 'textDecorationLine: task.done ? "line-through" : "none"'],
  ...
  ['order rows render a done checkbox', 'data-order-row-done-checkbox="order-row-done-checkbox"'],
  ['order rows can undo done tasks', 'task.done ? "Mark task not done" : "Mark task done"'],
  ['order rows done button persists through same handler', 'cardRect: cardElement?.getBoundingClientRect()'],
  ['order rows checkbox uses a checkmark only when done', 'task.done ? "✓" : ""'],
  ['order rows done button guards pointer interactions', 'onPointerDown={(event) => event.stopPropagation()}'],
  ['order rows done button guards mouse interactions', 'onMouseDown={(event) => event.stopPropagation()}'],
  ['order rows done button guards touch interactions', 'onTouchStart={(event) => event.stopPropagation()}'],
