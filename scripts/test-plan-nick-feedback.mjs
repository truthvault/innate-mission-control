...
assert.doesNotMatch(source, /label: "Order linked"/, 'Nick card cleanup: linked orders should not show a success pill on every card');
assert.doesNotMatch(source, /return \{ state: "needs-order" as OrderConnectionState, label: "Needs order",/, 'Nick card cleanup: do not show duplicate Needs order plus Needs order link pills');
assert.match(source, /const orderConnectionNeedsAttention = orderConnection\.state === "needs-order" \|\| orderConnection\.state === "possible";/, 'Nick card cleanup: only order-link problems should create card attention pills');
assert.match(source, /\{orderConnectionNeedsAttention && \(/, 'Nick card cleanup: hide linked/internal order status pills on normal cards');
assert.doesNotMatch(source, /taskActionPrompt\(task, orderConnection\.state === "needs-order"\)/, 'Nick card cleanup: do not add a second order-link prompt beside the order status pill');
