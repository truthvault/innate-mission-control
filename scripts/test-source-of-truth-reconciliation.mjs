...
const result = reconcileSourceOfTruth({
  ...
  monday: {
    ...
    orders: [
      ...
      { id: 504, customer: 'Legacy Monday Only', status: 'Booked', rawMondayStatus: 'Booked', xeroInvoiceNumber: null },
    ],
  },
  xero: {
    status: 'connected',
...
assert.equal(blair?.evidenceConfidence, 'verified');
...
assert.equal(kelven?.evidenceConfidence, 'review_needed');
...
assert.equal(abigail?.evidenceConfidence, 'missing_evidence');
assert.equal(abigail?.invoiceCount, 0);

assert.equal(result.unmatchedMondayOrders.length, 1, 'Monday-only mirror items are detected as missing Supabase canonical orders');
assert.equal(result.totals.missingSupabaseOrderCount, 1);
assert.equal(result.unmatchedMondayOrders[0].customer, 'Legacy Monday Only');
assert.ok(result.reportItems.some((item) => item.issue.includes('Monday mirror order has no matched Supabase canonical order') && item.confidence === 'review_needed'));
...
assert.match(markdown, /Missing Supabase canonical orders/);
assert.match(markdown, /Confidence: review_needed/);
