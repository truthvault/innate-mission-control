...
function displayName(lead) {
  const customerName = typeof lead.customerName === "string" ? lead.customerName.trim() : "";
  const contactName = typeof lead.contactName === "string" ? lead.contactName.trim() : "";
  const raw = customerName || contactName || "there";
  return raw
    .split("/")
    .map((part) => part.trim())
    .find(Boolean) || "there";
...
function draftContext(lead) {
  return [lead.sampleSpecies, lead.lastInteractionSummary, lead.nextAction, lead.notes]
    .filter(Boolean)
    .join(" \n")
    .toLowerCase();
}

function replacementSampleName(lead) {
  const text = [lead.sampleSpecies, lead.lastInteractionSummary, lead.nextAction, lead.notes]
    .filter(Boolean)
    .join(" \n");
  const westCoastRimuCountryBark = /west coast rimu\s*-?\s*country bark|rimu\s*-?\s*country bark/i.exec(text);
  if (westCoastRimuCountryBark) return "West Coast Rimu Country Bark";
  const sampleSpecies = typeof lead.sampleSpecies === "string" && lead.sampleSpecies.trim() ? lead.sampleSpecies.trim() : "timber";
  return `${sampleSpecies} sample`;
}

export function sampleDraftPrompt(lead) {
  const name = displayName(lead);
  const context = draftContext(lead);

  if (/\b(broken|damaged|replacement|replace|another piece)\b/i.test(context)) {
    const sampleName = replacementSampleName(lead);
    return `Hi ${name},\n\nSorry about that, and thanks for letting us know. We’ll send out a replacement ${sampleName} sample for you.\n\nI’ll get that sorted to the same address.\n\nCheers,\nGuido`;
  }
