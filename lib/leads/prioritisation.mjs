...
export function doToday(lead, today = todayKey()) {
  ...
  if (lead.priority === "hot" && isDueThisWeek(lead, today) && needsNextStep(lead)) return true;
  if (isCashflowQuote(lead) && needsNextStep(lead) && (isHighValue(lead) || isDueThisWeek(lead, today))) return true;
