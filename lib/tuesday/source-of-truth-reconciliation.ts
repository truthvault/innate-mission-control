...
export type ConfidenceLabel = "verified" | "probable" | "missing_evidence" | "review_needed";
...
export type DriftItem = {
  kind: "status_mismatch" | "missing_monday_mirror" | "missing_supabase_order" | "invoice_mismatch";
...
function supabaseMatchesMonday(order: SupabaseOrder, monday: MondayMirrorOrder) {
  const id = order.mondayItemId == null ? null : String(order.mondayItemId);
  if (id && id === String(monday.id)) return true;
  return norm(order.customerName) === norm(monday.customer);
}
...
export function reconcileSourceOfTruth(input: ReconciliationInput): ReconciliationResult {
  ...
  const orders = input.supabase.orders.map((order) => {
    ...
    const evidenceConfidence: ConfidenceLabel = hasXeroBankDisagreement || mondayDrift.length > 0
      ? "review_needed"
      : documents.length === 0
        ? "missing_evidence"
  ...
  const unmatchedMondayOrders = input.monday.orders.filter((monday) => !input.supabase.orders.some((order) => supabaseMatchesMonday(order, monday)));
  const unmatchedPayments = input.akahu.payments.filter((payment) => !usedPaymentIds.has(payment.transactionId));
  const reportItems: ReportItem[] = [];

  for (const [source, status] of Object.entries({ supabase: input.supabase, monday: input.monday, xero: input.xero, akahu: input.akahu })) {
    if (status.status !== "connected") {
      reportItems.push({
        severity: source === "supabase" ? "blocked" : "review",
        issue: `${source} evidence source is ${status.status}${status.error ? `: ${status.error}` : ""}.`,
        suggestedAction: "Treat affected reconciliation confidence as missing evidence until the read-only source is connected.",
  ...
  for (const order of orders) {
    if (order.mondayDrift.length > 0) {
      reportItems.push({ severity: "review", orderId: order.orderId, customerName: order.customerName, issue: `Supabase/Monday drift: ${order.mondayDrift.map((item) => item.message).join(" ")}`, suggestedAction: "Review mirror lag; keep Supabase canonical unless a human approves an update.", confidence: "review_needed", sources: order.mondayDrift.flatMap((item) => item.sources) });
    }
    if (order.financialStatus === "not_invoiced") {
      reportItems.push({ severity: "review", orderId: order.orderId, customerName: order.customerName, issue: "No Xero invoice/quote evidence matched this Supabase order.", suggestedAction: "Check whether this is a true pre-invoice order or a missing Xero/order link before using it in owner brief totals.", confidence: "missing_evidence", sources: [`supabase:orders.${order.orderId}`] });
    }
    if (["deposit_due", "part_paid", "overpaid", "review_needed"].includes(order.financialStatus)) {
      reportItems.push({ severity: order.financialStatus === "review_needed" || order.financialStatus === "overpaid" ? "review" : "info", orderId: order.orderId, customerName: order.customerName, issue: `${order.financialStatus.replace(/_/g, " ")}; balance due $${order.balanceDue.toFixed(2)}.`, suggestedAction: "Use as owner-brief evidence only. Do not mark paid from weak bank evidence.", confidence: order.financialStatus === "review_needed" || order.financialStatus === "overpaid" ? "review_needed" : order.evidenceConfidence, sources: [`supabase:orders.${order.orderId}`, ...order.invoiceNumbers.map((n) => `xero:${n}`)] });
    }
  }

  for (const monday of unmatchedMondayOrders) {
    reportItems.push({
      severity: "review",
      issue: `Monday mirror order has no matched Supabase canonical order: ${monday.customer}${monday.status ? ` (${monday.status})` : ""}.`,
      suggestedAction: "Review whether Supabase is missing an order or whether the Monday mirror item is stale/legacy before any writeback.",
      confidence: "review_needed",
      sources: [`monday:${monday.id}`],
    });
  }

  for (const payment of unmatchedPayments) {
    reportItems.push({ severity: "review", issue: `Unmatched deposit/payment ${payment.transactionId} for $${asMoney(payment.amount).toFixed(2)}${payment.payerName ? ` from ${payment.payerName}` : ""}.`, suggestedAction: "Review bank reference against orders before linking. Do not auto-mark paid.", confidence: "review_needed", sources: [`${payment.source}:${payment.transactionId}`] });
  }
  ...
  for (const fact of REVIEWED_CANONICAL_FACTS) {
    ...
    if (matched && !norm(matched.canonicalStatus).includes(norm(fact.expected).replace(/_/g, " "))) {
      reportItems.push({ severity: "review", orderId: matched.orderId, customerName: matched.customerName, issue: `Known current state guardrail: ${fact.note} Current Supabase status is ${matched.canonicalStatus ?? "unknown"}.`, suggestedAction: "Check Supabase before trusting Monday or financial evidence.", confidence: "review_needed", sources: [`supabase:orders.${matched.orderId}`] });
    }
  }
  ...
  return {
    ...
    totals: {
      ...
      driftCount,
      reviewCount,
      missingSupabaseOrderCount: unmatchedMondayOrders.length,
...
export function renderReconciliationMarkdown(result: ReconciliationResult): string {
  ...
  const cashLines = result.orders
    ...
    .map((order) => `- ${order.customerName}: bank matched $${order.bankAmountMatched.toFixed(2)}, Xero paid $${order.xeroAmountPaid.toFixed(2)}, balance $${order.balanceDue.toFixed(2)} (${order.financialStatus}; confidence ${order.evidenceConfidence}).`);
  const balanceLines = result.orders
    .filter((order) => order.balanceDue > MONEY_TOLERANCE && order.invoiceCount > 0)
    .map((order) => `- ${order.customerName}: balance due $${order.balanceDue.toFixed(2)} across ${order.invoiceCount} invoice(s) ${order.invoiceNumbers.join(", ") || "no invoice number"} (confidence ${order.evidenceConfidence}).`);
  const driftLines = result.orders
    .flatMap((order) => order.mondayDrift.map((drift) => `- ${order.customerName}: ${drift.message}`));
  const missingSupabaseLines = result.unmatchedMondayOrders.map((order) => `- ${order.customer}: Monday mirror item ${order.id}${order.status ? ` (${order.status})` : ""} has no matched Supabase canonical order.`);
  ...
  const reviewLines = result.reportItems
    ...
    .map((item) => `- ${item.customerName ? `${item.customerName}: ` : ""}${item.issue} Confidence: ${item.confidence}. Next: ${item.suggestedAction}`);
  ...
  return [
    ...
    `- Monday orders missing Supabase canonical match: ${result.totals.missingSupabaseOrderCount}`,
    ...
    ``,
    `## Missing Supabase canonical orders`,
    lineOrNone(missingSupabaseLines),
    ``,
    `## Unmatched deposits`,
