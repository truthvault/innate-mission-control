...
export function OrderPhotoTray({ orderId, embedded = false, onPhotoUploaded }: { orderId: number; embedded?: boolean; onPhotoUploaded?: () => void }) {
  ...
  return (
    <div style={{ border: `1px solid ${embedded ? "rgba(12,124,122,0.16)" : DT.border}`, background: embedded ? "rgba(255,255,255,0.62)" : DT.cardBg, borderRadius: embedded ? 9 : 12, padding: embedded ? 8 : 12, boxShadow: embedded ? "none" : "0 6px 18px rgba(34,32,26,0.035)" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
        <div>
          {!embedded && <div style={{ fontFamily: DT.sans, fontSize: 10, fontWeight: 900, textTransform: "uppercase", letterSpacing: "0.10em", color: DT.teal }}>Photos</div>}
        ...
        {!requested ? (
          <button
            ...
            style={{ border: `1px solid rgba(12,124,122,0.18)`, background: DT.tealSoft, color: DT.teal, borderRadius: 999, padding: "5px 8px", fontFamily: DT.sans, fontSize: 10, fontWeight: 900, cursor: "pointer" }}
          >
            Load photos
          </button>
        ) : (
          <label title={disabledReason || "Upload order photos"} style={{ border: `1px solid rgba(12,124,122,0.18)`, background: disabledReason || uploading ? "rgba(0,0,0,0.035)" : DT.tealSoft, color: disabledReason || uploading ? DT.textFaint : DT.teal, borderRadius: 999, padding: embedded ? "5px 8px" : "6px 9px", fontFamily: DT.sans, fontSize: 10, fontWeight: 900, cursor: disabledReason || uploading ? "not-allowed" : "pointer", whiteSpace: "nowrap" }}>
      ...
      {(status || disabledReason || !requested) && <div style={{ marginTop: 6, border: disabledReason ? "1px solid rgba(138,91,31,0.20)" : "none", background: disabledReason ? "rgba(250,204,21,0.10)" : "transparent", borderRadius: 8, padding: disabledReason ? "6px 8px" : 0, fontFamily: DT.sans, fontSize: 10, color: disabledReason ? DT.goldInk : DT.textMuted, fontWeight: 900, lineHeight: 1.3 }}>{status || disabledReason || "Photos load only when needed."}</div>}
      {requested && (
        <div style={{ marginTop: embedded ? 6 : 8, display: "grid", gridTemplateColumns: embedded ? "repeat(3, minmax(0, 1fr))" : "repeat(4, minmax(0, 1fr))", gap: 5 }}>
          {photos.map((photo) => (
            <div key={photo.pathname} style={{ position: "relative", aspectRatio: "1 / 1", borderRadius: 8, overflow: "hidden", border: `1px solid ${DT.border}`, background: DT.cardBg }}>
              ...
              <button
                ...
                style={{ position: "absolute", right: 4, top: 4, border: "1px solid rgba(154,59,47,0.16)", background: pendingDeletePathname === photo.pathname ? "rgba(154,59,47,0.88)" : "rgba(255,253,249,0.92)", color: deletingPathname === photo.pathname ? DT.textFaint : pendingDeletePathname === photo.pathname ? DT.cardBg : DT.textMuted, borderRadius: 999, minWidth: 30, height: 24, padding: "0 6px", fontFamily: DT.sans, fontSize: 9, fontWeight: 900, cursor: deletingPathname === photo.pathname ? "wait" : "pointer", lineHeight: "20px" }}
          ...
          {photos.length === 0 && <div style={{ gridColumn: "1 / -1", border: `1px dashed ${DT.border}`, background: "rgba(255,255,255,0.60)", borderRadius: 8, padding: embedded ? "6px 7px" : "8px 9px", fontFamily: DT.sans, fontSize: embedded ? 10 : 11, color: DT.textMuted, lineHeight: 1.3 }}>No photos yet.</div>}
        </div>
      )}
    </div>
  );
