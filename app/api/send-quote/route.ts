...
function htmlBody(payload: SendQuotePayload, shareUrl: string) {
  const dispatchWeek = formatDispatchWeek(new Date(), payload.totals.leadTimeWeeks);
  return `<!doctype html><html><body style="margin:0;padding:24px;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;background:#faf8f5;color:#141413;line-height:1.55"><div style="max-width:560px;margin:0 auto;background:#fff;padding:32px;border-radius:12px"><img src="${LOGO_URL}" alt="Innate Furniture" width="180" style="display:block;border:0;height:auto;width:180px;max-width:100%;margin-bottom:12px"><h1 style="font-size:20px">Benchtop quote · ${esc(payload.quoteNo)}</h1><p>${payload.path === "workshop" ? `New benchtop lead from <strong>${esc(payload.customer.name)}</strong>.` : `Hi ${esc(greetFirstName(payload.customer.name))}, here's your benchtop quote.`}</p><p><a href="${esc(shareUrl)}" style="display:inline-block;padding:12px 20px;background:#163832;color:#f3f0ee;text-decoration:none;border-radius:6px;font-weight:600">Open interactive quote</a></p><table style="width:100%;border-collapse:collapse;font-size:14px"><tr><td>Timber</td><td>${esc(payload.quote.species)}</td></tr><tr><td>Finish</td><td>${esc(finishLabel(payload.quote.finish))}</td></tr><tr><td>Colour</td><td>${esc(colourLabel(payload.quote.colour))}</td></tr><tr><td>Delivery</td><td>${esc(payload.totals.shipping.label)}${payload.totals.shipping.cost > 0 ? ` · ${esc(nzd(payload.totals.shipping.cost))}` : ""}</td></tr><tr><td>Dispatch</td><td>Estimated ${esc(dispatchWeek)}</td></tr><tr><td>Total</td><td><strong>${esc(nzd(payload.totals.grand))} incl GST</strong></td></tr></table><pre style="white-space:pre-wrap;font-family:inherit">${esc(panelRows(payload.quote))}</pre><p style="color:#14141399;font-size:12px">Innate Furniture · 281 Queen Elizabeth II Drive, Christchurch</p></div></body></html>`;
}
